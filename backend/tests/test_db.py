...
async def test_tx_commits(seed):
    ...
    try:
        ...
        row = await db.one("SELECT name FROM zones WHERE id = $1", "r_tmp")
...
async def test_tx_rolls_back_on_error(seed):
    ...
    try:
        ...
        row = await db.one("SELECT id FROM zones WHERE id = $1", "r_bad")
    ...
    try:
        ...
        with pytest.raises(ValueError):
            async with db.tx():
                await inner()
                raise ValueError("boom")
        row = await db.one("SELECT id FROM zones WHERE id = $1", "r_inner")

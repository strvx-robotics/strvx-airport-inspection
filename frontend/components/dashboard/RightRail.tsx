  ...
  return (
    <section className={cn("overflow-hidden rounded-md", CARD)}>
      <div className={cn("flex items-center justify-between px-4 py-3", BAR)}>
        <h3 className="text-[13px] font-semibold text-[#e7eaec]">Zoning map</h3>
        <Badge tone="gray">Preview</Badge>
      </div>
      <div className="space-y-2.5 bg-[#0f1214] px-4 py-4">
        {runways.map((r) => {
          ...
          return (
            <div key={r.runway.id} className="flex items-center gap-3">
              <span className="w-14 shrink-0 font-mono text-[10px] text-[#737a7f]">{r.runway.designation}</span>
              <div className="h-6 flex-1">
                <div
                  className="relative h-full rounded-sm border border-[#262b2f] bg-[#16191c]"
                  style={{ width: `${w}%` }}
                >
                  <div
                    ...
                    style={{
                      height: 2,
                      opacity: 0.6,
                      background: "repeating-linear-gradient(90deg,#c2c8cc 0 10px,transparent 10px 20px)",
                    }}
                  />
                </div>
              </div>
      ...
      <div className="border-t border-dashed border-[#343a3f] px-4 py-2.5">
        <p className="text-center font-mono text-[10px] text-[#5b6166]">
...
export function RecentWorkOrders({ tickets }: { tickets: Ticket[] }) {
  return (
    <section className={cn("overflow-hidden rounded-md", CARD)}>
      <div className={cn("flex items-center justify-between px-4 py-3", BAR)}>
        <h3 className="text-[13px] font-semibold text-[#e7eaec]">Recent work orders</h3>
      ...
      {tickets.length === 0 ? (
        ...
        tickets.map((t) => (
          <Link
            key={t.id}
            href={`/ticket/${t.id}`}
            className="block border-b border-[#262b2f] px-4 py-2.5 last:border-b-0 hover:bg-[#16191c]"
          >
            <div className="flex items-center justify-between gap-2">
              <span className="font-mono text-[12px] text-[#e7eaec]">{t.id}</span>
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <span className="truncate text-[11px] text-[#9aa1a6]">
                {CATEGORY[t.category]} · {t.zone}
              </span>
              <span className="ml-auto shrink-0 font-mono text-[10px] text-[#5b6166]">
  ...
  return (
    <section className={cn("overflow-hidden rounded-md", CARD)}>
      <div className={cn("px-4 py-3", BAR)}>
        <h3 className="text-[13px] font-semibold text-[#e7eaec]">Recent passes</h3>
      </div>
      {rows.length === 0 ? (
        ...
        rows.map((i) => {
          ...
          return (
            <div
              key={i.id}
              className="flex items-center gap-3 border-b border-[#262b2f] px-4 py-2.5 last:border-b-0"
            >
              <span
                className={cn("h-1.5 w-1.5 shrink-0 rounded-full", active ? "bg-[#e7eaec]" : "bg-[#343a3f]")}
              />
              <div className="min-w-0">
                <p
                  className={cn(
                    "font-mono text-[12px] leading-tight",
                    active ? "text-[#e7eaec]" : "text-[#9aa1a6]",
                  )}
                >
                  {fmtInTz(i.scheduledTime, undefined, { month: "short", day: "numeric", year: "numeric" })}
                </p>
                <p className="mt-0.5 text-[10px] leading-tight text-[#737a7f]">{INSPECTION_WINDOW[i.window]}</p>
              </div>

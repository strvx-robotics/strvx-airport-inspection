...
import { ChevronLeft, Layers, Undo2, X } from "lucide-react";
import type { Runway } from "@/lib/types";
import { cn } from "@/lib/cn";
import { BTN, BTN_PRIMARY, CARD, INPUT, MUTED } from "@/lib/vstyle";

export type ZoneDrawStep = "details" | "plot" | "confirm";

export function InspectionZonesModal({
  open,
  step,
  runways,
  lockRunwayId,
  ...
  const showRunwayPick = !lockRunwayId;
    ...
    <div className="pointer-events-none absolute inset-0 z-20 flex justify-end p-3">
      ...
      >
        <header className="flex items-center gap-2 border-b border-[#dbdfe3] px-3 py-2.5">
          ...
          <span className="text-[13px] font-semibold text-[#181b1e]">Draw inspection zone</span>
          {step !== "details" && (
            <button
              type="button"
              onClick={() => onStep(step === "confirm" ? "plot" : "details")}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          {step === "details" && (
            <div className="space-y-3">
              <p className={cn("text-[12px] leading-relaxed", MUTED)}>
                Name the zone and pick a runway. You&apos;ll draw its boundary on the map next.
              </p>
              <Field label="Zone name">
                <input
                  value={name}
                  onChange={(e) => onName(e.target.value)}
                  placeholder="Zone C · rollout"
              ...
              {showRunwayPick ? (
                ...
                selectedRunway && (
                  <p className={cn("text-[12px]", MUTED)}>
                    Runway: <span className="font-medium text-[#181b1e]">{selectedRunway.name}</span>
                  </p>
                )
              )}
              <button
          ...
          {step === "plot" && (
            <div className="space-y-3">
              <p className={cn("text-[12px] leading-relaxed", MUTED)}>
                ...
                <span className="font-medium text-[#181b1e]">{selectedRunway?.name ?? "the runway"}</span>.
          ...
          {step === "confirm" && (
            <div className="space-y-3">
              <p className={cn("text-[12px] leading-relaxed", MUTED)}>Name the zone and review before saving.</p>
              <Field label="Zone name">
                <input
                  value={name}
                  onChange={(e) => onName(e.target.value)}
              ...
              <dl className="space-y-2 rounded-md border border-[#dbdfe3] bg-[#f3f5f7] px-3 py-2.5 text-[12px]">
                <div>
                  <dt className="font-mono text-[10px] uppercase tracking-wide text-[#6b7176]">Runway</dt>
                  <dd className="text-[#181b1e]">{selectedRunway?.name ?? runwayId}</dd>
                </div>
                <div>

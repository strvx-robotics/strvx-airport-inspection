...
interface ImageRow { id: string; job_id: Str; flight_id: Str; zone_id: string; boundary_id: Str; file_url: string; gps_lat: Num; gps_lng: Num; station_m: Num; lateral_offset_m: Num; alt_m: Num; heading_deg: Num; geom_confidence: string; timestamp: string; captured_at: Str; source_file: Str; metadata_json: Str; created_at: string }
...
function toImage(r: ImageRow): Image {
  return { id: r.id, jobId: u(r.job_id), flightId: u(r.flight_id), zoneId: r.zone_id, boundaryId: u(r.boundary_id), fileUrl: r.file_url, gps: gps(r.gps_lat, r.gps_lng), stationM: u(r.station_m), lateralOffsetM: u(r.lateral_offset_m), altM: u(r.alt_m), headingDeg: u(r.heading_deg), geomConfidence: r.geom_confidence as GeomConfidence, timestamp: r.timestamp, capturedAt: u(r.captured_at), sourceFile: u(r.source_file), metadata: r.metadata_json ? (JSON.parse(r.metadata_json) as Record<string, unknown>) : undefined, createdAt: r.created_at };
}

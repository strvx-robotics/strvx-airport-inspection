...
assemble + inspect the dataset without training.
    ...
    python -m rl.finetune --category pavement --epochs 80
    python -m rl.finetune --category fod --build-only
    python -m rl.finetune --selfcheck            # dataset-assembly logic, no train
"""
...
from rl import feedback as fb

CATEGORIES = ["fod", "pavement", "marking", "lighting"]
APP_URL = os.environ.get("APP_URL", "http://localhost:3000")
MODELS_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "models")
CACHE = os.path.join(os.path.dirname(__file__), "artifacts", "img_cache")
...
def run_finetune(category, source=None, epochs=80, imgsz=640, base=None, promote_margin=0.0,
    ...
    out_dir = out_dir or os.path.join(os.path.dirname(__file__), "artifacts", f"dataset_{category}")
...
def _selfcheck() -> None:
    ...
    up = os.path.join(os.path.dirname(os.path.dirname(__file__)), "..", "frontend", "public", "uploads")

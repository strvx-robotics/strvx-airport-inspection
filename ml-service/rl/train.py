...
detector threshold policy -> promotion gate (held-out) -> save versioned artifacts.
    ...
    python -m rl.train                                   # pulls the app export
    python -m rl.train --source feedback.jsonl
    python -m rl.train --source http://localhost:3000/api/feedback-export
"""
...
from rl.policy import ThresholdPolicy, WriterBandit, load_policies, save_policies
...
def train(source=None, artifacts=ARTIFACTS, holdout=0.25, min_samples=8,
    ...
    if len(wsamples) >= min_samples:
        ...
        if incumbent is None:
            ...
            from rl.eval.run import judge_gate
...
def _threshold_eval(dsamples, thr: ThresholdPolicy) -> dict:
    ...
    from rl.policy import DEFAULT_THRESHOLDS
